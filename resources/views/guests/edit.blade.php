...
@section('title', 'Edit Tamu - KATIBER')

@section('content')
    <div class="max-w-3xl mx-auto bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border dark:border-slate-700">
        <h2 class="text-xl font-bold mb-6 border-b dark:border-slate-700 pb-2 text-slate-800 dark:text-white">Edit Data Tamu
        </h2>
        ...
        <form action="{{ route('guests.update', $guest->id) }}" method="POST">
                ...
                <select name="agenda_id" required
                    ...
                    @foreach ($agendas as $agenda)
                        <option value="{{ $agenda->id }}"
                            {{ old('agenda_id', $guest->agenda_id) == $agenda->id ? 'selected' : '' }}>
                            {{ \Carbon\Carbon::parse($agenda->date)->format('d/m/Y') }} - {{ $agenda->name }}
                        </option>
                    @endforeach
                </select>
                @error('agenda_id')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="institution" value="{{ old('institution', $guest->institution) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-1">Perwakilan / Jabatan
                        (Opsional)</label>
                    <input type="text" name="role" value="{{ old('role', $guest->role) }}"
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-1">Kontak / No HP
                        (Opsional)</label>
                    <input type="text" name="contact" value="{{ old('contact', $guest->contact) }}"
