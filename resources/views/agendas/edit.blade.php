...
@section('title', 'Edit Agenda - KATIBER')
...
@section('content')
    <div class="mx-auto max-w-4xl">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-800 dark:text-white">Edit Agenda</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Perbarui rincian kegiatan organisasi.</p>
            </div>
        </div>
        ...
        <div
            ...
            <form action="{{ route('agendas.update', $agenda->id) }}" method="POST" autocomplete="off"
                x-data="{
                    initialPic: '{{ old('person_in_charge', $agenda->person_in_charge) }}',
                    pic: '',
                    code: '{{ old('agenda_code', $agenda->agenda_code) }}',
                    collabDivisions: [],
                    init() {
                        // Cek apakah data lama adalah kolaborasi (mengandung ' x ')
                        if (this.initialPic.includes(' x ')) {
                            this.pic = 'Kolaborasi / Lintas Divisi';
                            this.collabDivisions = this.initialPic.split(' x ').map(s => s.trim());
                        } else {
                            this.pic = this.initialPic;
                        }
                    },
                    prefixes: {
                        @foreach($divisions as $div)
                        '{{ $div->name }}': '{{ strtoupper($div->abbreviation) }}-',
                        @endforeach 'Kolaborasi / Lintas Divisi': 'KOLAB-'
                    },
                    updatePrefix() {
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Kode
                            Agenda</label>
                        <input type="text" x-model="code" name="agenda_code" required autocomplete="off"
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Nama
                            Agenda</label>
                        <input type="text" name="name" value="{{ old('name', $agenda->name) }}" required
                            autocomplete="off"
                            class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 dark:[color-scheme:dark]">
                        @error('name')
                            <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                ...
                <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-3">
                    ...
                    <div>
                        ...
                        <select name="type" required
                            ...
                            <option value="Rapat Internal"
                                {{ old('type', $agenda->type) == 'Rapat Internal' ? 'selected' : '' }}>Rapat Internal
                            </option>
                            <option value="Kegiatan / Event"
                                {{ old('type', $agenda->type) == 'Kegiatan / Event' ? 'selected' : '' }}>Kegiatan / Event
                            </option>
                            <option value="Pleno / Muskom"
                                {{ old('type', $agenda->type) == 'Pleno / Muskom' ? 'selected' : '' }}>Pleno / Muskom
                            </option>
                            <option value="Lainnya" {{ old('type', $agenda->type) == 'Lainnya' ? 'selected' : '' }}>Lainnya
                            </option>
                        </select>
                        @error('type')
                            <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="Terjadwal"
                                {{ old('status', $agenda->status) == 'Terjadwal' ? 'selected' : '' }}>Terjadwal</option>
                ...
                <!-- Menu Checklist Kolaborasi untuk Edit -->
                ...
                <div class="mt-5">
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500">{{ old('notes', $agenda->notes) }}</textarea>
                </div>
                ...
                <div class="mt-5 flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
                    <input type="checkbox" id="is_public" name="is_public" value="1"
                        {{ old('is_public', $agenda->is_public) ? 'checked' : '' }}
