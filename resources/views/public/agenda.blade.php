...
@section('meta_description', 'Jadwal agenda dan kalender kegiatan KATIBER — Keluarga Mahasiswa Tebing Tinggi Bersatu — yang akan dan telah berlangsung.')
        ...
        <section class="relative overflow-hidden bg-gradient-to-br from-navy-800 via-navy-900 to-navy-950">
            ...
            <div class="relative mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div class="reveal">
                        <span class="inline-block rounded-full bg-white/10 px-3.5 py-1 text-xs font-semibold uppercase tracking-wider text-white ring-1 ring-inset ring-white/15">Kalender Kegiatan</span>
                        <h1 class="mt-3 text-3xl font-extrabold text-white">Agenda Kegiatan</h1>
                        <p class="mt-2 text-sm text-navy-100">Jadwal kegiatan KATIBER Lhokseumawe - Aceh Utara yang akan dan
                            telah berlangsung.</p>
                    </div>

                    {{-- Toggle Kalender / Daftar --}}
                    <div class="reveal flex w-fit rounded-lg bg-white p-1 shadow-sm dark:bg-slate-800" style="--reveal-delay:120ms">
                        <button @click="tab = 'calendar'" type="button"
                            ...
                            :class="tab === 'calendar' ? 'bg-navy-700 text-white shadow-sm' :
                        ...
                        <button @click="tab = 'list'" type="button"
                            class="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-semibold transition"
                            :class="tab === 'list' ? 'bg-navy-700 text-white shadow-sm' :
                                'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75"
                                stroke="currentColor" class="h-4 w-4">
                                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <section x-show="tab === 'calendar'" x-cloak class="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
            <div
                ...
                <div class="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-800">
                    <h2 class="text-lg font-bold text-slate-800 dark:text-white" x-text="monthNames[month] + ' ' + year">
                    </h2>
                ...
                <div class="grid grid-cols-7 auto-rows-fr gap-px bg-slate-100 dark:bg-slate-800">
                    ...
                    <template x-for="date in no_of_days">
                        <div
                            class="relative min-h-[90px] bg-white p-2 transition hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800/50 sm:min-h-[110px]">
                            <span class="inline-flex h-6 w-6 items-center justify-center rounded-full text-sm font-semibold"
                                :class="isToday(date) ? 'bg-navy-700 text-white' : 'text-slate-700 dark:text-slate-300'"
                                x-text="date"></span>
                            ...
                            <div
                                ...
                                <div class="min-w-0">
                                    ...
                                    @if ($agenda->person_in_charge)
                                        <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">PJ:
                                            {{ $agenda->person_in_charge }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
            ...
            @else
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($pastAgendas as $agenda)
                        <div
                            class="theme-transition flex items-start gap-4 rounded-2xl border border-slate-100 bg-white p-5 opacity-80 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                            <div
                                class="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-xl bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                                <span
                                    class="text-lg font-bold leading-none">{{ \Carbon\Carbon::parse($agenda->date)->format('d') }}</span>
                                <span
                                    class="text-[11px] uppercase leading-none">{{ \Carbon\Carbon::parse($agenda->date)->translatedFormat('M') }}</span>
