...
@section('title', 'Edit Event Unggulan - KATIBER')
...
@section('content')
    <div class="mx-auto max-w-3xl">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-800 dark:text-white">Edit Event Unggulan</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ $event->title }}</p>
            </div>
        </div>
        ...
        <div class="theme-transition rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8"
            x-data="eventForm()">
            <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data" @submit="beforeSubmit">
                @csrf
                @method('PUT')
    ...
    @include('events._form-script', ['event' => $event])
@endsection
