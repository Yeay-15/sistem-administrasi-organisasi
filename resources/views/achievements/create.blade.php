...
@section('title', 'Tambah Prestasi - KATIBER')
...
@section('content')
    <div class="mx-auto max-w-2xl">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-800 dark:text-white">Tambah Prestasi</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Data ini akan tampil di bagian "Prestasi Mahasiswa" pada Beranda.</p>
            </div>
        </div>
        ...
        <div class="theme-transition rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8"
            ...
            <form action="{{ route('achievements.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-xs text-slate-400">Maks. 3MB.</p>
                            @error('photo')
                ...
                <div class="mt-5">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Juara 2 Debat B. Inggris"
                ...
                <div class="mt-5">
                    ...
                    <input type="text" name="achiever_name" value="{{ old('achiever_name') }}" required placeholder="Contoh: Tiara Azzahra & Matilda F."
                        class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500">
                    @error('achiever_name')
                        <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="mt-5">
                    ...
                    <input type="text" name="description" value="{{ old('description') }}" placeholder="Contoh: Tingkat Nasional 2025"
                        class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500">
                    @error('description')
                        <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                    @enderror
