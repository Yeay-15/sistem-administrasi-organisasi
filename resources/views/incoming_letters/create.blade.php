...
@section('title', 'Catat Surat Masuk - KATIBER')
...
@section('content')
    <div class="mx-auto max-w-4xl">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-800 dark:text-white">Catat Surat Masuk</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Simpan data surat eksternal beserta arsipnya.</p>
            </div>
        </div>
            ...
            <form action="{{ route('incoming-letters.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="received_date" value="{{ old('received_date') }}" required
                ...
                <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="sender" value="{{ old('sender') }}" placeholder="Contoh: BEM UNIMAL" required
                            class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 dark:[color-scheme:dark]">
                        @error('sender')
                            <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="addressed_to" value="{{ old('addressed_to') }}" placeholder="Contoh: Ketua Umum" required
                            class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 dark:[color-scheme:dark]">
                        @error('addressed_to')
                            <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-5 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-800/60">
                    ...
                    <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">Format: PDF. Maksimal: 5MB.</p>
                    @error('file_path')
                        <p class="mt-1.5 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-5">
                    <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Keterangan / Disposisi (Opsional)</label>
