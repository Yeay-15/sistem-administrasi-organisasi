...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 min-h-screen theme-transition"
    ...
    <div class="lg:flex lg:min-h-screen">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <nav class="sidebar-scroll flex-1 space-y-1 overflow-y-auto px-3 py-4">
                ...
                @endcan
                @can('view_non_pengurus')
                    @include('layouts.partials.nav-item', [
                        'route' => 'non-pengurus.index',
                        'pattern' => 'non-pengurus.*',
                        'label' => 'Anggota Non-Pengurus',
                        'icon' => 'identification',
                    ])
                @endcan
                @can('view_committees')
    ...
    <script>
        // Setiap halaman dimuat ulang (navigasi biasa, bukan SPA), sidebar
        // ikut ter-render ulang dari scroll teratas — jadi kalau menu aktif
        // letaknya di bagian bawah daftar, ia jadi tersembunyi di luar area
        // yang terlihat. Script ini menggeser sidebar supaya menu yang lagi
        // aktif langsung kelihatan, tanpa scroll manual.
        (function () {
            var activeItem = document.querySelector('.sidebar-scroll [data-nav-active="true"]');
            if (activeItem) {
