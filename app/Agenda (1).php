                ...
                <div class="theme-transition rounded-2xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"
                    ...
                    <form action="{{ route('kepanitiaan.anggota.store', $kepanitiaan->id) }}" method="POST" @submit="handleSubmit($event)">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div class="md:col-span-2" x-show="mode === 'existing'">
                                <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Orang</label>
                                <select @change="memberId = $event.target.value" :required="mode === 'existing'"
                                    ...
                                    @foreach ($availableMembers as $m)
                                        <option value="{{ $m->id }}">{{ $m->name }} ({{ $m->membership_type }}{{ $m->student_id ? ' · ' . $m->student_id : '' }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div x-show="mode === 'new'">
                            ...
                            <div>
                                ...
                                <select name="position_category" x-model="positionCategory" required
                                    class="block w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                            ...
                            <div x-show="positionCategory === 'Ketua Bidang' || positionCategory === 'Anggota Bidang'">
                ...
                @can('manage_committees')
                    <form action="{{ route('kepanitiaan.agenda.store', $kepanitiaan->id) }}" method="POST" class="border-t border-slate-100 p-4 dark:border-slate-800">
                        ...
                        <div class="flex gap-2">
                            <select name="agenda_id" required class="block w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                            ...
                            <button type="submit" class="shrink-0 rounded-lg bg-slate-800 px-3 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 dark:bg-slate-100 dark:text-slate-900">+</button>
                        </div>
                    </form>
                @endcan
            </div>
